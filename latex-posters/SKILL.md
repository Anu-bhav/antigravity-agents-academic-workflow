...
**Review Script** (Available in `scripts/review_poster.py`):
```bash
...
**GUIDING SKILL**:
- **academic-standards**: Poster content must be high-quality and verified.

## Next Steps

- **Draft Content**: Use `research-paper-writer` for text.
- **Compile**: Use `latex-manual` tools.
- **Review**: Run `python scripts/review_poster.py`.
