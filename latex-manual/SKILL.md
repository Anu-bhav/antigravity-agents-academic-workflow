...
- **compile_latex.py** - Compile LaTeX document (supports bibtex, clean)
- **clean_latex.py** - Remove auxiliary files
- **bibtex_workflow.py** - Run complete BibTeX workflow
...
**Standard:**
```bash
python scripts/compile_latex.py document.tex
```

**With Bibliography:**
```bash
python scripts/compile_latex.py document.tex --bibtex
```

**Clean after compilation:**
```bash
python scripts/compile_latex.py document.tex --clean
```
...
**GUIDING SKILL**:
- **academic-standards**: All documents must adhere to styles defined here.

## Next Steps

- **Draft Content**: Use `research-paper-writer` or `academic-writing`.
- **Review**: Check PDF output quality.
- **Finish**: If document is complete, you are done.
